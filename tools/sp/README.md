...
- **Thin orchestration**: sp invokes `saltc` with `--root <dir>` for the project's `src/` (and the stdlib, when found) and `--dep <name>=<entry>` for each resolved dependency, never touching compiler internals. `use <name>` loads the dependency's `entry` file, and `use <name>.a.b` loads `a/b.salt` (or `a/b/mod.salt`) beside it
...
- ✅ Importing from dependencies: `use <dep>.…` compiles for path, version and transitive dependencies (`sp build`, `sp check`)
...
| `sp build` fails with `no published versions found for 'X'` | `X` is a version dependency (for example from `sp add X`) that isn't in `~/.salt/publish/` | Run `sp publish` in X's directory |
| `could not find imported module 'X.…'` when importing a dependency | `X` isn't a key under `[dependencies]` in `salt.toml`, so sp doesn't pass it to `saltc` | Add it (`sp add X`, or `X = { path = "…" }`). The key must be the name you import |
| `dependency 'X' entry point not found` | `entry` in X's `salt.toml` (default `src/main.salt`) doesn't name X's root module. Libraries made by `sp new --lib` before this fix have this problem | Set `entry = "src/lib.salt"` (or wherever X's root module is) under `[package]` in X's `salt.toml` |
