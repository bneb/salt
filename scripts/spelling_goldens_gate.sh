...
DIR="$ROOT/salt-front/tests/fixtures/spelling_goldens"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
check cache_repro 0 'Cache__new_64' '_SIZE' \
  "$DIR/const_generic_overflow_repro.salt"
check shadow_repro 0 'Box2__make_5' '(_NODE|Node_[0-9])' \
  "$DIR/adv_shadow_real_struct.salt"

# RT6 probes. FLIP rows change in WS-R3 -- see header.
check neg_turbofish 0 'S_-7' 'S_main__-' "$DIR/rt_probe_neg_turbofish.salt"  # FLIP[T-a] LANDED: single identity main__S_-7; ghost family forbidden
check neg_two_values 0 'struct_main__S_-13' 'S_main__-' \
  "$DIR/rt_probe_neg_turbofish_two_values.salt"     # FLIP[T-a] stronger golden (RT req): -7 AND -13 => one decl each
check neg_two_values_7 0 'struct_main__S_-7' 'S_main__-' \
  "$DIR/rt_probe_neg_turbofish_two_values.salt"     # symmetric require leg (grep -qE cannot demand BOTH in one row)
check digit_ident_rail 0 'main__S_7' '' \
  "$DIR/rt_probe_digit_ident_regression.salt"       # guard must not over-reject digit-containing idents
check hex_norm 0 'struct_main__S_16' 'S_main__-' \
  "$DIR/rtv_hex_lit.salt"                           # hex literal normalizes via evaluator value
check underscore_norm 0 'struct_main__S_1000' '' \
  "$DIR/rtv_underscore_lit.salt"                    # underscore literal normalizes via evaluator value
check leading_zero 0 '(Z_7|Z__mk_7)' '' "$DIR/rt_probe_leading_zero.salt"
check unary_plus nonzero '' '' "$DIR/rt_probe_unary_plus.salt"              # parser E002; router unreached
check overflow_literal nonzero '' '' "$DIR/rt_probe_overflow_literal.salt"   # FLIP[T-b] LANDED: [E003] refusal cites digits; no MLIR artifact -> no O_K ghost, no ptr-typed call
check i64max 0 '9223372036854775807' '' "$DIR/rt_probe_i64max.salt"
...
check nb4_receiver_compiles 0 'main__len_of_T' '(Unsupported explicit cast|Unresolved generic)' "$DIR/nb4_generic_vec_receiver.salt"
...
check nb2_compiles 0 'main__drain_i64' '(Unresolved generic|Undefined struct)' "$DIR/nb2_vec_identity_match.salt"  # FLIP[NB-5c]: arity-tolerant unify binds U
...
check i64min_neg_literal 0 'constant -9223372036854775808 : i64' \
  'constant 9223372036854775807 : i64' "$DIR/probe_i64min_neg.salt"
